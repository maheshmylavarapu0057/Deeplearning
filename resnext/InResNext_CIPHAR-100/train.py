...
net = resnext101_32x8d() # Specify num_classes=100 for CIFAR-100 training
...
def train_epoch(net,optimizer,trainloader,testloader,it,control_dict,global_output_filename = "out.txt"):
    ...
    if True:
        ...
        write_file_and_close(global_output_filename,
                             ...
                             .format(int(info[0]), info[1], (info[0] / info[1]), info[2] / info[1])
                             )
...
class NN_SGDTrainer(object):
    ...
    def train(self,model_name="test"):
        ...
        if self.iter_time == 300: # Change to 300 for CIFAR100 training
...
trainloader,testloader = get_cifar100(batch_size) # Change this line to get_cifar100 for CIFAR-100 training

sgd_para = {"lr":1e-3, "weight_decay":0.0001, "momentum":0.9}

Trainer = NN_SGDTrainer(net, sgd_para, trainloader, testloader, {150:1e-1,225:1e-2,300:1e-3}, sys.argv[2] + model_name + '.txt') # Change the learning rates to {150:1e-1,225:1e-2,300:1e-3} for CIFAR-100 training 

for i in range(300): # Change the total number of epochs to 300 for CIFAR-100 training
    Trainer.train()
