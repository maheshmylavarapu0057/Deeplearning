...
testset = torchvision.datasets.CIFAR100( # Change this to CIFAR100 if conducting FGSM attack on CIFAR-100
...
for inputs, labels in now_dataloader:
    ...
    eps = torch.ones_like(inputs) * eps_
       #epe=eps.cuda()
    eps = torch.stack((eps[:,0,:,:] / 0.2023, eps[:,1,:,:] / 0.1994, eps[:,2,:,:] / 0.2010), dim=1)
