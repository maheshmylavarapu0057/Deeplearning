...
class InResNet(nn.Module):
    ...
    def __init__(self,block,layers,pretrain=False,num_classes=10,stochastic_depth=False,PL=0.5,noise_level=0.001,noise=False):
        ...
        self.relu=nn.ReLU(inplace=True)
        self.pretrain=pretrain
#        self.ks=nn.ParameterList([nn.Parameter(torch.Tensor(1).uniform_(0.8, 0.9))for i in range(layers[0]+layers[1]+layers[2])])
        self.ks=nn.ParameterList([nn.Parameter(torch.Tensor(1).uniform_(0.1, 0.2))for i in range(layers[0]+layers[1]+layers[2])]) # Use this line for \lambda-In-ResNet; for 164-layer experiments, use [0.8, 0.9] for In-ResNet or [0.1, 0.2] for \lambda-In-ResNet
    ...
    def print_ks(self):
        ha = []
        for i in self.ks: ha.append((functional.relu(i)).item()) #Change functional.relu(1-i) into functional.relu(i) for \lambda-In-ResNet experiments 
        return str(ha)
